...
StorageData load_data(const std::filesystem::path& path) {
    ...
    while (doc_csv(product_stream, row)) {
        ++product_record;
        require_columns(row, product_header, "san_pham.csv", product_record);
        ...
        product.stock = parse_int(row[col_stock], "san_pham.csv", product_record, "stock");
        product.reorder_level = parse_int(row[col_reorder], "san_pham.csv", product_record, "reorder_level");
    ...
    while (doc_csv(order_stream, row)) {
        ++order_record;
        require_columns(row, order_header, "don_hang.csv", order_record);
        ...
        item.name = "";
        item.quantity = parse_int(row[order_col_quantity], "don_hang.csv", order_record, "quantity");
