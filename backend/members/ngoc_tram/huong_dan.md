...
| `Build/Search Trie` PASS giả khi sản phẩm đầu tiên có `name` rỗng | code lấy `prefix` trực tiếp từ `first_product.name` mà không xử lý trường hợp tên rỗng, dẫn tới prefix rỗng -> test mất khả năng phân biệt đúng/sai. | fallback sang dùng `sku` làm nguồn lấy prefix khi `name` rỗng | Tìm prefix `SKU` thành công (matches.size()=3) | 
